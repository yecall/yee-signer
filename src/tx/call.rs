        ...
        S: Serializer,
    {
        match self {
            Call::Timestamp(call) => match call {
                timestamp::Call::Set(call) => {
                    serialize_call(serializer, timestamp::MODULE, timestamp::SET, call)
                }
            },
            Call::Consensus(call) => match call {
                consensus::Call::ReportMisbehavior(call) => serialize_call(
                    serializer,
                    consensus::MODULE,
                    consensus::REPORT_MISBEHAVIOR,
                    call,
                ),
                consensus::Call::NoteOffline(call) => {
                    serialize_call(serializer, consensus::MODULE, consensus::NOTE_OFFLINE, call)
                }
                consensus::Call::Remark(call) => {
                    serialize_call(serializer, consensus::MODULE, consensus::REMARK, call)
                }
                consensus::Call::SetHeapPages(call) => serialize_call(
                    serializer,
                    consensus::MODULE,
                    consensus::SET_HEAP_PAGES,
                    call,
                ),
                consensus::Call::SetCode(call) => {
                    serialize_call(serializer, consensus::MODULE, consensus::SET_CODE, call)
                }
                consensus::Call::SetStorage(call) => {
                    serialize_call(serializer, consensus::MODULE, consensus::SET_STORAGE, call)
                }
                consensus::Call::KillStorage(call) => {
                    serialize_call(serializer, consensus::MODULE, consensus::KILL_STORAGE, call)
                }
            },
            Call::Pow(call) => match call {
                pow::Call::SetPowInfo(call) => {
                    serialize_call(serializer, pow::MODULE, pow::SET_POW_INFO, call)
                }
            },
            Call::Indices(_call) => unreachable!(),
            ...
            Call::Sharding(call) => match call {
                sharding::Call::SetShardInfo(call) => {
                    serialize_call(serializer, sharding::MODULE, sharding::SET_SHARD_INFO, call)
                }
            },
            Call::Crfg(call) => match call {
                crfg::Call::UpdateAuthorities(call) => {
                    serialize_call(serializer, crfg::MODULE, crfg::UPDATE_AUTHORITIES, call)
                }
                crfg::Call::ForceUpdateAuthorities(call) => serialize_call(
                    serializer,
                    crfg::MODULE,
            ...
            Call::FinalityTracker(call) => match call {
                finality_tracker::Call::WriteFinalizedLog(call) => serialize_call(
                    serializer,
            ...
            Call::Assets(call) => match call {
                assets::Call::Issue(call) => {
                    serialize_call(serializer, assets::MODULE, assets::ISSUE, call)
                }
                assets::Call::Transfer(call) => {
                    serialize_call(serializer, assets::MODULE, assets::TRANSFER, call)
                }
            },
            Call::Relay(call) => match call {
                relay::Call::Transfer(call) => {
                    serialize_call(serializer, relay::MODULE, relay::TRANSFER, call)
                }
            },
            Call::Storage(call) => match call {
                storage::Call::Store(call) => {
                    serialize_call(serializer, storage::MODULE, storage::STORE, call)
                }
            },
            Call::Sudo(call) => match call {
                sudo::Call::Sudo(call) => {
                    serialize_call(serializer, sudo::MODULE, sudo::SUDO, call)
                }
                sudo::Call::SetKey(call) => {
                    serialize_call(serializer, sudo::MODULE, sudo::SET_KEY, call)
                }
            },
        }
    }
                ...
                V: SeqAccess<'de>,
            {
                ...
                let call = match module {
                    ...
                    consensus::MODULE => match method {
                        consensus::REPORT_MISBEHAVIOR => Call::Consensus(
                            consensus::Call::ReportMisbehavior(seq_next_element(seq, &self)?),
                        ),
                        consensus::NOTE_OFFLINE => Call::Consensus(consensus::Call::NoteOffline(
                            seq_next_element(seq, &self)?,
                        )),
                        consensus::REMARK => {
                            Call::Consensus(consensus::Call::Remark(seq_next_element(seq, &self)?))
                        }
                        consensus::SET_HEAP_PAGES => Call::Consensus(
                            consensus::Call::SetHeapPages(seq_next_element(seq, &self)?),
                        ),
                        consensus::SET_CODE => {
                            Call::Consensus(consensus::Call::SetCode(seq_next_element(seq, &self)?))
                        }
                        consensus::SET_STORAGE => Call::Consensus(consensus::Call::SetStorage(
                            seq_next_element(seq, &self)?,
                        )),
                        consensus::KILL_STORAGE => Call::Consensus(consensus::Call::KillStorage(
                            seq_next_element(seq, &self)?,
                        )),
                        unknown => {
                            return Err(de::Error::invalid_value(
                    ...
                    indices::MODULE => {
                        return Err(de::Error::invalid_value(
                            Unexpected::Unsigned(indices::MODULE as u64),
                            &self,
                        ));
                    }
                    balances::MODULE => match method {
                        balances::TRANSFER => {
                    ...
                    sharding::MODULE => match method {
                        sharding::SET_SHARD_INFO => Call::Sharding(sharding::Call::SetShardInfo(
                            seq_next_element(seq, &self)?,
                        )),
                        unknown => {
                            return Err(de::Error::invalid_value(
                    ...
                    crfg::MODULE => match method {
                        ...
                        crfg::FORCE_UPDATE_AUTHORITIES => Call::Crfg(
                            crfg::Call::ForceUpdateAuthorities(seq_next_element(seq, &self)?),
                        ),
                        unknown => {
                            return Err(de::Error::invalid_value(
                                Unexpected::Unsigned(unknown as u64),
                    ...
                    finality_tracker::MODULE => match method {
                        finality_tracker::WRITE_FINALIZED_LOG => {
                            Call::FinalityTracker(finality_tracker::Call::WriteFinalizedLog(
                                seq_next_element(seq, &self)?,
                            ))
                        }
                        unknown => {
                    ...
                    sudo::MODULE => match method {
                        sudo::SUDO => Call::Sudo(sudo::Call::Sudo(seq_next_element(seq, &self)?)),
                        sudo::SET_KEY => {
                    ...
                    match key {
                        ...
                        Field::Params => {
                            ...
                            call = Some(match module {
                                ...
                                consensus::MODULE => match method {
                                    consensus::REPORT_MISBEHAVIOR => Call::Consensus(
                                        consensus::Call::ReportMisbehavior(map.next_value()?),
                                    ),
                                    consensus::NOTE_OFFLINE => Call::Consensus(
                                        consensus::Call::NoteOffline(map.next_value()?),
                                    ),
                                    consensus::REMARK => {
                                        Call::Consensus(consensus::Call::Remark(map.next_value()?))
                                    }
                                    consensus::SET_HEAP_PAGES => Call::Consensus(
                                        consensus::Call::SetHeapPages(map.next_value()?),
                                    ),
                                    consensus::SET_CODE => {
                                        Call::Consensus(consensus::Call::SetCode(map.next_value()?))
                                    }
                                    consensus::SET_STORAGE => Call::Consensus(
                                        consensus::Call::SetStorage(map.next_value()?),
                                    ),
                                    consensus::KILL_STORAGE => Call::Consensus(
                                        consensus::Call::KillStorage(map.next_value()?),
                                    ),
                                    unknown => {
                                        return Err(de::Error::invalid_value(
                                            Unexpected::Unsigned(unknown as u64),
                                ...
                                sharding::MODULE => match method {
                                    sharding::SET_SHARD_INFO => Call::Sharding(
                                        sharding::Call::SetShardInfo(map.next_value()?),
                                    ),
                                    unknown => {
                                        return Err(de::Error::invalid_value(
                                            Unexpected::Unsigned(unknown as u64),
                                ...
                                crfg::MODULE => match method {
                                    ...
                                    crfg::FORCE_UPDATE_AUTHORITIES => Call::Crfg(
                                        crfg::Call::ForceUpdateAuthorities(map.next_value()?),
                                    ),
                                    unknown => {
                                        return Err(de::Error::invalid_value(
                                            Unexpected::Unsigned(unknown as u64),
                                ...
                                finality_tracker::MODULE => match method {
                                    finality_tracker::WRITE_FINALIZED_LOG => Call::FinalityTracker(
                                        finality_tracker::Call::WriteFinalizedLog(
                                            map.next_value()?,
                                        ),
                                    ),
                                    unknown => {
                                ...
                                sudo::MODULE => match method {
                                    sudo::SUDO => Call::Sudo(sudo::Call::Sudo(map.next_value()?)),
                                    sudo::SET_KEY => {
...
fn serialize_call<S: Serializer, T: Serialize>(
    serializer: S,
    module: u8,
    method: u8,
    params: &T,
...
fn seq_next_element<'de, SA: SeqAccess<'de>, V: Visitor<'de>, T: Deserialize<'de>>(
    mut seq: SA,
    visitor: &V,
) -> Result<T, SA::Error> {
    let call = seq
...
mod tests {
    ...
    #[test]
    fn test_call_serde() {
        let call = Call::Timestamp(timestamp::Call::Set(timestamp::Set { now: Compact(12) }));
