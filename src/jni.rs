...
(env: JNIEnv, _jclass: JClass, module: jint, method: jint, params: jbyteArray, error: jbyteArray)-> jlong {

	let run = || -> SignerResult<jlong> {
		let params = env.convert_byte_array(params).map_err(|_| "jni error")?;
		let call = export::common_build_call(module as u8, method as u8, &params)?;
		Ok(call as jlong)
	};
	error_result_jni(run, 0, &env, error)
}
...
(env: JNIEnv, _jclass: JClass, call: jlong, module: jint, method: jint, error: jbyteArray) {
	let run = || -> SignerResult<()> {
		export::call_free(call as *mut c_uint, module as u8, method as u8)
	};
...
(env: JNIEnv, _jclass: JClass, secret_key: jbyteArray, nonce: jlong, period: jlong, current: jlong, current_hash: jbyteArray, call: jlong, module: jint, method: jint, error: jbyteArray) -> jlong {
	let run = || -> SignerResult<jlong> {
		...
		let result = export::common_build_tx(secret_key, nonce, period, current, current_hash,
										  call as *mut c_uint,module as u8, method as u8)?;
...
(env: JNIEnv, _jclass: JClass, tx: jlong, module: jint, method: jint, error: jbyteArray) {
	let run = || -> SignerResult<()> {
		export::tx_free(tx as *mut c_uint, module as u8, method as u8)
	};
...
(env: JNIEnv, _jclass: JClass, tx: jlong, module: jint, method: jint, error: jbyteArray) -> jlong {
	let run = || -> SignerResult<i64> {
		let len = export::tx_encode(tx as *mut c_uint, module as u8, method as u8)?.len();
		Ok(len as i64)
	};
...
(env: JNIEnv, _jclass: JClass, tx: jlong, module: jint, method: jint, buffer: jbyteArray, error: jbyteArray) {
	let run = || -> SignerResult<()> {
		let result = export::tx_encode(tx as *mut c_uint, module as u8, method as u8)?
			.iter().map(|x| *x as i8).collect::<Vec<_>>();
		env.set_byte_array_region(buffer, 0, &result).map_err(|_| "jni error")?;
		Ok(())
	};
...
(env: JNIEnv, _jclass: JClass, tx: jlong, module: jint, method: jint, current_hash: jbyteArray, error: jbyteArray) {
	let run = || -> SignerResult<()> {
		...
		export::common_verify_tx(tx as *mut c_uint, module as u8, method as u8, &current_hash)
	};
